...
ProjectFrame::ProjectFrame ( const wxString& title, const wxPoint& pos, const wxSize& size)
		    : wxFrame((wxFrame *)NULL, -1, title, pos, size)
  {
	...
	menuStack->Append( ID_ShowHeadStack,	wxT("Show Head"),wxT("Show Head") );
	menuStack->Append( ID_ShowTailStack, 	wxT("Show Tail"),wxT("Show Tail") );
	...
	*/
	...
	SetStatusText( wxT("     Mellissa Marie"), 1 );
	
	//Put something in the Third section of the status bar
	SetStatusText( wxT("    412002117"), 2 );
...
string makeRecord(int id, string first, string last, string place, string timeOfYear, string payType)
    {
        string record = "";
	...
	record.append(heading);
        ...
        while (!infile.eof())//read all the records in the file
                ...
                if (!infile.eof())
		  {
			...
			record = makeRecord(id, a, b, c, d, e);
						
						
			myqueue->enqueue(id, a, b, c, d, e);
		  }
...
/*	Record dataRec;
		...
		DataDialog *datadialog = new DataDialog( wxT("COMP2115 - Data Entry for Queue"), 
		...
		if (datadialog->ShowModal() == wxID_OK )
			...
			queue->enqueue(id, first, last, place, timeOfYear, payType, bookingType);                            
...
void ProjectFrame::OnCreateDeque(wxCommandEvent& event)
  {
	...
	char comma;
        ...
        int kount = 0;// Keep a count of the records
        ...
        while (!infile.eof())//read all the records in the file
            {
			 ...
			 kount++;
                ...
                if (!infile.eof())
					...
					// Alternate adding the records between the head and tail fo the List
			if (kount % 2 == 0)
                            {
                                record.append("\t...Added to Head\n");
                                deque->enqueueHead(id, first, last, place, timeOfYear, payType, bookingType);
                            }
			else
			    {
				record.append("\t...Added to Tail\n");
                                deque->enqueueTail(id, first, last, place, timeOfYear, payType, bookingType);	
			    }
...
void ProjectFrame::OnDisplayDeque(wxCommandEvent& event)
  {
	...
	records.append(deque->displayDQ());
...
void ProjectFrame::OnDequeueHeadDeque(wxCommandEvent& event)
  {
	...
	MainEditBox->Clear();
	 ...
	 // Create a label for the displayed records
        ...
        heading.append("==================================================================================================\n");
	record.append(heading);
	record.append(deque->dequeueHead());
...
void ProjectFrame::OnDequeueTailDeque(wxCommandEvent& event)
  {
	...
	MainEditBox->Clear();
	 ...
	 // Create a label for the displayed records
        ...
        heading.append("==================================================================================================\n");
	record.append(heading);
	record.append(deque->dequeueTail());
...
void ProjectFrame::OnDisplayStack(wxCommandEvent& event)
  {
	...
	records.append(stack->displayStack());
...
void ProjectFrame::OnPreorderRBT(wxCommandEvent& event)
  {
	...
	records.append(rbt->preOrderRBT());
...
void ProjectFrame::OnPostorderRBT(wxCommandEvent& event)
  {
	...
	records.append(rbt->postOrderRBT());
...
void ProjectFrame::OnDeleteRBT(wxCommandEvent& event)
  {
    Record dataRec;
		...
		DeleteDialog *deletedialog = new DeleteDialog( wxT("COMP2115 - Data Deletion for RBT"), 
							 wxPoint(200,200), wxSize(420,180) );
